...
# 可视化函数
# ----------------------
def plot_training_losses(stage1_losses, stage3_losses, filename=None):
    """绘制训练损失曲线"""
    if filename is None:
        filename = os.path.join(BASE_DIR, 'visualizations', 'stage1_vs_stage3_losses.png')
...
def plot_mae_comparison(stage1_mae, stage2_mae, stage4_mae, filename=None):
    """绘制MAE对比图"""
    if filename is None:
        filename = os.path.join(BASE_DIR, 'visualizations', 'mae_across_stages.png')
...
def plot_improvement(stage2_mae, stage4_mae, filename=None):
    """绘制改进对比图"""
    if filename is None:
        filename = os.path.join(BASE_DIR, 'visualizations', 'improvement_after_reinforcement.png')
    plt.figure(figsize=(8, 6))
...
def plot_learning_curves(stage1_losses, stage3_losses, filename=None):
    """绘制学习曲线"""
    if filename is None:
        filename = os.path.join(BASE_DIR, 'visualizations', 'learning_curves.png')
    plt.figure(figsize=(10, 6))
...
def plot_mae_trend(maes, stages, filename=None):
    """绘制MAE趋势图"""
    if filename is None:
        filename = os.path.join(BASE_DIR, 'visualizations', 'mae_trend.png')
    plt.figure(figsize=(8, 6))
...
def plot_prediction_vs_actual(model, data, filename=None):
    pass
...
def main():
    ...
    random_embedding_path = os.path.join(project_root, 'embedding', 'random_word_embedding.npz')
    random_word_data = np.load(random_embedding_path, allow_pickle=True)
    ...
    # 游戏数据使用单词embedding
    # embedding文件夹位于项目根目录
    games_embedding_path = os.path.join(project_root, 'embedding', 'games_word_embedding.npz')
    games_word_data = np.load(games_embedding_path, allow_pickle=True)
    ...
    # 3.5 保存阶段3模型
    stage3_model_file = os.path.join(BASE_DIR, "stage3_reinforced_model.npz")
    ...
    # 为了兼容可视化函数，设置默认的stage1和stage2结果
    stage1_mse = 0.0
    stage1_mae = 0.0
    stage1_losses = []
    stage2_mse = 0.0
    stage2_mae = 0.0
    stage1_model_file = "stage3_reinforced_model.npz"
    ...
    # 保存所有结果
    results_file = os.path.join(BASE_DIR, "four_stage_experiment_results.npz")
    ...
    # 生成实验总结文档
    summary_file = os.path.join(BASE_DIR, "four_stage_experiment_summary.md")
